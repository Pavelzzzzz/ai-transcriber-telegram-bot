...
from database.models import User, Transcription, AdminLog, BotStatistics, UserRole, TranscriptionType
...
class AdminService:
    ...
    def get_transcription_statistics(self) -> Dict[str, Any]:
        ...
        successful_transcriptions = self.db.query(Transcription).filter(
            Transcription.status == 'completed'
        ).count()
        failed_transcriptions = self.db.query(Transcription).filter(
            Transcription.status == 'failed'
        ).count()

        # Статистика по типам транскрипций
        text_to_audio_count = self.db.query(Transcription).filter(
            Transcription.transcription_type == TranscriptionType.TEXT_TO_AUDIO
        ).count()
        audio_to_text_count = self.db.query(Transcription).filter(
            Transcription.transcription_type == TranscriptionType.AUDIO_TO_TEXT
        ).count()

        # Среднее время обработки
        avg_processing_time = self.db.query(Transcription.processing_time).filter(
            Transcription.status == 'completed',
            Transcription.processing_time.isnot(None)
        ).all()

        avg_time = sum([time[0] for time in avg_processing_time]) / len(avg_processing_time) if avg_processing_time else 0

        return {
            'total_transcriptions': total_transcriptions,
            'successful_transcriptions': successful_transcriptions,
            'failed_transcriptions': failed_transcriptions,
            'success_rate': round((successful_transcriptions / total_transcriptions * 100), 2) if total_transcriptions > 0 else 0,
    ...
    def get_user_transcriptions(self, user_id: int, limit: int = 10) -> List[Transcription]:
        ...
        if user:
            ...
            ).order_by(Transcription.created_at.desc()).limit(limit).all()
        return []
    ...
    def get_daily_statistics(self, days: int = 7) -> List[BotStatistics]:
        ...
        return self.db.query(BotStatistics).filter(
            BotStatistics.date >= start_date
        ).order_by(BotStatistics.date.desc()).all()
    ...
    def log_transcription(self, user_id: int, status: str, transcription_type: TranscriptionType = TranscriptionType.TEXT_TO_AUDIO,
                         processing_time: int = None, error_message: str = None, text_length: int = 0,
                         input_text: str = None, input_audio_path: str = None, output_audio_path: str = None,
