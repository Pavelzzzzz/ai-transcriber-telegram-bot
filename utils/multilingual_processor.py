...
class MultilingualTextProcessor:
    ...
    def correct_grammar(self, text: str, language: str) -> tuple[str, list[TextCorrection]]:
        ...
        if language == 'ru':
            ...
            grammar_rules = {
                r'\bмного\b\s+([а-яё]{2,})ая\b': lambda m: f"много {m.group(1)[0].lower()}{m.group(1)[1:] if len(m.group(1)) > 1 else ''}",
                r'\bмного\b\s+([а-яё]{2,})ий\b': lambda m: f"много {m.group(1)[0].lower()}{m.group(1)[1:] if len(m.group(1)) > 1 else ''}",
            }
    ...
    def correct_punctuation_and_capitalization(self, text: str, language: str) -> tuple[str, list[TextCorrection]]:
        ...
        if capital_rules.get('sentence_start'):
            ...
            for i, sentence in enumerate(sentences):
                ...
                if sentence and len(sentence) > 0:
                    ...
                    if not any(sentence.lower().startswith(abbrev) for abbrev in capital_rules.get('abbreviations', [])):
    ...
    def generate_recommendations(self, text: str, stats: dict[str, int], language: str, corrections_count: int) -> list[str]:
    ...
    def process_text(self, text: str) -> TextAnalysisResult:
        ...
        corrected, punctuation_corrections = self.correct_punctuation_and_capitalization(corrected, language)
