...
from telegram import Update, Message, Voice, Photo
...
class SafeMediaProcessor:
    ...
    async def safe_message_check(self, update: Update) -> Optional[Message]:
    ...
    def safe_voice_check(self, update: Update) -> Optional[Voice]:
    ...
    def safe_photo_check(self, update: Update) -> Optional[Photo]:
    ...
    async def send_success_result(self, update: Update, title: str, result_data: str, audio_path: str = None) -> bool:
        ...
        try:
            ...
            return self.send_error_with_fallback(
                update,
                "❌ **Ошибка результата**",
                f"Не удалось отправить результат",
                f"Попробуйте еще раз"
            )
    
    async def safe_download_file(self, update: Update, file_id: str, suffix: str = ".tmp") -> Optional[str]:
        ...
        try:
            ...
            await self.send_error_with_fallback(
                ...
                f"Попробуйте отправить файл другим способом"
            )
            return None
