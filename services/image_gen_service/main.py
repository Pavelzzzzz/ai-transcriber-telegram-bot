...
def start_health_server(port=8005):
...
class ImageGenerationService(BaseService):
    ...
    def _send_notification(self, user_id: str, message: str):
        try:
            ...
            notification = {"user_id": user_id, "message": message, "type": "image_generation"}
        ...
        if self._result_producer is None:
            try:
                ...
                raise KafkaProducerError(
                    f"Failed to create result producer: {e}", "image_gen_service"
                )
