...
class TestImageGenerationProcessor:
    ...
    async def test_generate_image_mock(self, mock_makedirs, processor):
        with patch('services.image_gen_service.processor.get_available_models', return_value=['sd15']):
            with patch('services.image_gen_service.processor.MODELS_CONFIG', {
                ...
                with patch('services.image_gen_service.processor.StableDiffusionPipeline') as mock_class:
    ...
    async def test_generate_image_with_metadata(self, mock_makedirs, processor):
        ...
        with patch('services.image_gen_service.processor.get_available_models', return_value=['sdxl']):
            with patch('services.image_gen_service.processor.MODELS_CONFIG', {
                'sdxl': {'model_id': 'stabilityai/stable-diffusion-xl-base-1.0', 'min_vram_gb': 8}
            }):
                with patch('services.image_gen_service.processor.ASPECT_RATIO_SIZES', {'16:9': (1024, 576)}):
                    mock_pipeline = Mock()
                    mock_image = Mock()
                    mock_result = Mock()
                    mock_result.images = [mock_image]
                    ...
                    with patch('services.image_gen_service.processor.StableDiffusionXLPipeline') as mock_class:
                        mock_class.from_pretrained.return_value = mock_pipeline
