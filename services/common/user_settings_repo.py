...
def update_user_settings(user_id: int, **kwargs) -> Optional[UserSettings]:
    valid_fields = ['image_model', 'image_style', 'aspect_ratio', 'num_variations', 'negative_prompt']
...
def get_user_image_history(user_id: int, limit: int = 10) -> list:
    try:
        with get_db() as db:
            return db.query(ImageGenerationHistory)\
