...
class TestSettingsHandlers:
    ...
    @patch('services.bot_service.settings_handlers.MODELS_CONFIG', {
...
class TestNegativePromptHandlers:
    ...
    async def test_handle_negative_prompt_input_skip(self, mock_update):
        from services.bot_service.settings_handlers import handle_negative_prompt_input, PENDING_NEGATIVE_PROMPT
...
class TestSettingsCallbackQuery:
    ...
    async def test_settings_callback_reset(self, mock_reset):
        from services.bot_service.settings_handlers import settings_callback, PENDING_NEGATIVE_PROMPT
