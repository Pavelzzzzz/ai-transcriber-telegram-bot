...
def get_settings_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton(f"🔢 Вариаций: {current_variations}", callback_data="settings:variations"),
        ],
        [
            InlineKeyboardButton(f"📝 Негативный промпт: {'Задан' if settings.negative_prompt else 'Не задан'}", 
...
async def settings_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    available_models = get_available_models()
    model_name = get_model_display_name(current_model) if current_model in available_models else f"{current_model} ⚠️"
    style_name = get_style_display_name(current_style) if current_style else "Без стиля"
    aspect_name = ASPECT_RATIO_NAMES.get(current_aspect, current_aspect)
...
async def show_model_selection(query, user_id: int):
    ...
    for model_id in MODELS_CONFIG.keys():
        ...
        keyboard.append([InlineKeyboardButton(button_text, callback_data=f"settings:model:{model_id}")])
    
    keyboard.append([InlineKeyboardButton("⬅️ Назад", callback_data="settings:back")])
    
    text = "**Выберите модель:**\n\n"
    text += "⚠️ - модель недоступна на текущем оборудовании\n"
...
async def show_style_selection(query, user_id: int):
    ...
    for style_id, config in STYLES_CONFIG.items():
        ...
        keyboard.append([InlineKeyboardButton(button_text, callback_data=f"settings:style:{style_id}")])
    
    keyboard.append([InlineKeyboardButton("⬅️ Назад", callback_data="settings:back")])
    
    await query.edit_message_text("**Выберите стиль:**", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
async def show_aspect_selection(query, user_id: int):
    ...
    for aspect_id, name in ASPECT_RATIO_NAMES.items():
        ...
        keyboard.append([InlineKeyboardButton(button_text, callback_data=f"settings:aspect:{aspect_id}")])
    
    keyboard.append([InlineKeyboardButton("⬅️ Назад", callback_data="settings:back")])
    
    await query.edit_message_text("**Выберите размер:**", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
async def show_variations_selection(query, user_id: int):
    ...
    for num in [1, 2, 3, 4]:
        ...
        keyboard.append([InlineKeyboardButton(button_text, callback_data=f"settings:variations:{num}")])
    
    keyboard.append([InlineKeyboardButton("⬅️ Назад", callback_data="settings:back")])
    
    await query.edit_message_text("**Выберите количество вариаций:**", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
async def handle_negative_prompt_input(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int) -> bool:
    ...
    await update.message.reply_text(
        f"✅ Негативный промпт сохранен: `{negative_prompt or 'Не задан'}`",
        parse_mode="Markdown"
    )
    return True
...
async def handle_settings_model_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if model not in available_models:
        await query.edit_message_text(f"⚠️ Модель {model} недоступна на текущем оборудовании")
        return
