...
from ..common import KafkaProducerError, TaskMessage, TaskType, kafka_config
...
class TaskProducer:
    ...
    def send_task(self, task: TaskMessage) -> bool:
        try:
            ...
            logger.info(f"Task {task.task_id} sent to {record_metadata.topic}:{record_metadata.partition}")
            return True
    ...
    def create_ocr_task(self, user_id: int, chat_id: int, file_path: str, metadata: dict = None) -> TaskMessage:
        task = TaskMessage(
            task_id="",
    ...
    def create_transcribe_task(self, user_id: int, chat_id: int, file_path: str, metadata: dict = None) -> TaskMessage:
        task = TaskMessage(
            task_id="",
    ...
    def create_image_gen_task(self, user_id: int, chat_id: int, prompt: str, metadata: dict = None) -> TaskMessage:
        task = TaskMessage(
            task_id="",
